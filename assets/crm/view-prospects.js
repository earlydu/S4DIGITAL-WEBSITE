...
async function load() {
  ...
  if (!result.items.length) {
    ...
    if (g) g.onclick = async () => { (await import('./nav.js?v=12')).go('import'); };
...
async function newProspect() {
  const { modal } = await import('./ui.js?v=12');
