...
/** PostgREST escapes commas and parens inside values by quoting them. */
const pgVal = v => {
...
function pgClause(c) {
  if (c.or) return `or(${c.or.map(pgClause).join(',')})`;
  const col = c.col;
  switch (c.op) {
    case 'isnull': return `or(${col}.is.null,${col}.eq.)`;
    case 'notnull': return `and(${col}.not.is.null,${col}.neq.)`;
    case 'in': return (c.val || []).length ? `${col}.in.(${(c.val).map(pgVal).join(',')})` : `${col}.is.null`;
    case 'nin': return (c.val || []).length ? `${col}.not.in.(${(c.val).map(pgVal).join(',')})` : `${col}.not.is.null`;
    case 'like': return `${col}.ilike.${pgVal('*' + c.val + '*')}`;
    case 'neq': return `${col}.neq.${pgVal(c.val)}`;
    case 'gt': return `${col}.gt.${pgVal(c.val)}`;
    case 'gte': return `${col}.gte.${pgVal(c.val)}`;
    case 'lt': return `${col}.lt.${pgVal(c.val)}`;
    case 'lte': return `${col}.lte.${pgVal(c.val)}`;
    default: return `${col}.eq.${pgVal(c.val)}`;
  }
}
...
function pgParams(opts) {
  ...
  if (opts.search && opts.search.term) {
    ...
    ands.push(`or(${opts.search.cols.map(c => `${c}.ilike.${pgVal(t)}`).join(',')})`);
  }
  for (const a of ands) {
