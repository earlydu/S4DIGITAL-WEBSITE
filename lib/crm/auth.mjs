...
export const RESET_MINUTES = Math.floor(RESET_MS / 60000);

/**
 * A reset token needs no table. It is signed with the server secret and carries
 * the account's current token_version, so the moment the password changes the
 * version moves on and every outstanding link stops working. That makes it
 * single-use without storing anything.
 */
export function makeResetToken(user) {
  const exp = String(Date.now() + RESET_MS);
  const nonce = randomBytes(8).toString('hex');
  const payload = `r.${user.id}.${user.token_version || 0}.${exp}.${nonce}`;
  return `${payload}.${sign(payload)}`;
}

export async function userForResetToken(token) {
  const parts = String(token || '').split('.');
  if (parts.length !== 6 || parts[0] !== 'r') return null;
  const [, id, ver, exp, nonce, mac] = parts;
  if (!configured()) return null;
  if (!constantEq(mac, sign(`r.${id}.${ver}.${exp}.${nonce}`))) return null;
  if (Number(exp) <= Date.now()) return null;

  const user = await first('users', { where: [{ col: 'id', op: 'eq', val: id }] });
  if (!user || !user.active) return null;
  if (Number(user.token_version || 0) !== Number(ver)) return null;   // already used
  return user;
}
