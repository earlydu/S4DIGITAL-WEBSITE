...
import { getSettings, CLOSED_STAGES, CONTACT_PRIORITY, STAGE } from './settings.mjs';
...
export const OUTCOMES = {
  no_answer: {
    label: 'No Answer', key: '1', stage: STAGE.REACHED_OUT, callStatus: 'no_answer',
    retry: 'noAnswerDays', attempt: true, group: 'attempt',
  },
  gatekeeper: {
    label: 'Gatekeeper', key: '2', stage: STAGE.REACHED_OUT, callStatus: 'gatekeeper',
    retry: 'gatekeeperDays', attempt: true, group: 'attempt',
  },
  decision_maker: {
    label: 'Spoke to Decision Maker', key: '3', stage: STAGE.ENGAGED, callStatus: 'contacted',
    retry: 'decisionMakerDays', attempt: true, reached: true, conversation: true, group: 'conversation',
    prompt: 'followup',
  },
  follow_up: {
    label: 'Follow Up', key: '4', stage: STAGE.FOLLOW_UP, callStatus: 'contacted',
    attempt: true, reached: true, conversation: true, group: 'conversation',
    needsFollowUp: true,
  },
  meeting_booked: {
    label: 'Meeting Booked', key: '5', stage: STAGE.MEETING, callStatus: 'contacted',
    attempt: true, reached: true, conversation: true, group: 'conversation',
    needsMeeting: true,
  },
  qualified: {
    label: 'Qualified', stage: STAGE.QUALIFIED, callStatus: 'contacted',
    attempt: true, reached: true, conversation: true, group: 'conversation',
  },
  proposal: {
    label: 'Offer Made', stage: STAGE.OFFER, callStatus: 'contacted',
    attempt: true, reached: true, conversation: true, group: 'conversation',
  },
  not_interested: {
    label: 'Not Interested', stage: STAGE.NOT_NOW, callStatus: 'not_interested',
    attempt: true, reached: true, conversation: true, group: 'conversation',
  ...
  won: {
    label: 'Client Won', stage: STAGE.WON, callStatus: 'contacted',
    attempt: true, reached: true, conversation: true, group: 'conversation', parks: true,
  },
  lost: {
    label: 'Not Now', stage: STAGE.NOT_NOW, callStatus: 'contacted',
    attempt: true, reached: true, conversation: true, group: 'conversation',
    parks: true, closedReason: 'Not now',
  },
};
 ...
 *
 * A short pattern like "TED" is a set of initials, and a plain substring test
 * blocks "Associated Controls", "United Air" and "Protected Systems" along with
 * it, silently, which is the worst way to lose a prospect. Short patterns have
 * to match a whole word; longer ones match as a whole phrase.
 */
export function matchesExclusion(name, patterns) {
  ...
  for (const p of patterns) {
    const raw = p.pattern || p;
    const pn = norm(raw);
    if (!pn) continue;
    if (n === pn) return raw;

    if (!pn.includes(' ') && pn.length <= 5) {
      if (words.has(pn)) return raw;
      continue;
    }

    const escaped = pn.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&');
    if (new RegExp(`(^| )${escaped}( |$)`).test(n)) return raw;
  }
  return null;
}
...
export async function buildQueue({ userId, day = today(), size } = {}) {
  ...
  if (picks.length < want) {
    const pool = await select('companies', {
      where: [
        ...base,
        { col: 'stage', op: 'in', val: [STAGE.NEW, STAGE.TO_CALL] },
