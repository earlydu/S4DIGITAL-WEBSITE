...
import { mailReady, sendMail, resetEmail } from './mail.mjs';
...
/** Origin for links in email. Falls back to the live site. */
function originOf(req) {
  const ref = (req.headers && (req.headers.origin || req.headers.referer)) || '';
  try { return new URL(ref).origin; } catch { return process.env.CRM_SITE_URL || 'https://s4digi.com'; }
}
...
function prospectWhere(f = {}) {
  ...
  if (f.won) where.push({ col: 'stage', op: 'eq', val: STAGE.WON });
  if (f.lost) where.push({ col: 'stage', op: 'eq', val: STAGE.NOT_NOW });
  return where;
}
...
export async function handleCrm({ action, req, body, secure }) {
  ...
  if (action === 'forgot') {
    ...
    const user = email ? await auth.findByEmail(email) : null;

    // Always the same answer, whether or not the account exists. Otherwise this
    // endpoint becomes a way to find out who has one.
    const vague = {
      ok: true,
      message: 'If that address has an account, a reset link is on its way. Check your inbox.',
    };

    if (!user || !user.active) return ok(vague);

    if (!mailReady()) {
      return bad(
        'Email is not set up on the server, so a reset link cannot be sent. ' +
        'Reset it from the command line instead: node tools/crm.mjs set-password ' + user.email,
        503);
    }

    const origin = originOf(req);
    const link = `${origin}/sales/reset?token=${encodeURIComponent(auth.makeResetToken(user))}`;
    const mail = resetEmail({ name: user.name, link, minutes: auth.RESET_MINUTES });
    try {
  ...
  if (action === 'reset-check') {
    const user = await auth.userForResetToken(body.token);
    return ok({ valid: Boolean(user), email: user ? user.email : '' });
  }

  if (action === 'reset') {
    const user = await auth.userForResetToken(body.token);
    if (!user) {
      return bad('That reset link has expired or has already been used. Ask for a new one.', 401);
    }
    try {
    ...
    // setPassword bumps token_version, so the link is now spent. Sign them in.
    const fresh = await auth.findByEmail(user.email);
    return ok({ ok: true, user: auth.publicUser(fresh) },
      { 'Set-Cookie': auth.cookieHeader(auth.makeToken(fresh), { secure }) });
  }
    ...
    switch (action) {
      ...
      case 'dashboard': {
        ...
        const [stats, score] = await Promise.all([
          dashboard({ userId, day }),
          scoreboard({ day, deep: true }),
        ]);
        return ok({ ...stats, scoreboard: score });
      }

      case 'scoreboard':
        return ok(await scoreboard({ day: body.day || today(), deep: Boolean(body.deep) }));

      /* ---------------------------------------------------------------- queue */

      case 'queue': {
        const day = body.day || today();
        const [q, score] = await Promise.all([getQueue({ userId, day }), scoreboard({ day })]);
        return ok({ ...q, scoreboard: score });
      }
      ...
      case 'prospect-stage': {
        ...
        await insert('activities', {
          ...
          note: `${company.stage || STAGE.NEW} to ${stage}`, detail: stage, occurred_at: nowISO(),
        });
        return ok({ company: saved });
      }
      ...
      case 'pipeline': {
        ...
        return ok({ stages: settings.stages, guide: STAGE_GUIDE, columns: out });
      }
